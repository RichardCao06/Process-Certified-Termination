"""Deterministic, non-enforcing checks for the P2 Shadow foundation.

Until PCT-P2-D03 is approved, every finding is descriptive. The registry
suggests possible failure codes and recommendations but does not promote any
new online Hard Gate.
"""
...
Check = Callable[
    [CandidateStopSnapshot, AppendOnlyEventLog, EvidenceLedger],
    list[Finding],
]


def _event_ids_for_evidence(
    ledger: EvidenceLedger, evidence_ids: tuple[str, ...]
) -> tuple[str, ...]:
    event_ids: list[str] = []
...
def check_verified_without_valid_evidence(
    ...
    for obligation in snapshot.obligation_states:
        ...
        if not valid:
            event_ids = (
                (obligation.last_transition_event_id,)
                if obligation.last_transition_event_id
                else ()
            )
            findings.append(
                Finding(
                    ...
                    category="EVIDENCE",
                    message=(
                        f"{obligation.obligation_id} is VERIFIED without current "
                        "goal-revision-matched evidence"
                    ),
                    event_ids=event_ids,
...
def check_stale_evidence_referenced(
    snapshot: CandidateStopSnapshot,
    _: AppendOnlyEventLog,
    ledger: EvidenceLedger,
) -> list[Finding]:
        ...
        if stale:
            findings.append(
                Finding(
                    ...
                    category="EVIDENCE",
                    message=(
                        f"{obligation.obligation_id} references stale, invalidated, "
                        "or wrong-revision evidence"
                    ),
...
def check_authoritative_failure_not_propagated(
    snapshot: CandidateStopSnapshot,
    _: AppendOnlyEventLog,
    ledger: EvidenceLedger,
) -> list[Finding]:
        ...
        if failures and obligation.state != "FAILED":
            ...
            findings.append(
                Finding(
                    ...
                    category="PROCESS",
                    message=(
                        f"{obligation.obligation_id} remains {obligation.state} "
                        "despite a current authoritative FAIL"
                    ),
...
def check_unresolved_hard_obligation_at_completion(
    snapshot: CandidateStopSnapshot,
    _: AppendOnlyEventLog,
    __: EvidenceLedger,
) -> list[Finding]:
    ...
    unresolved = tuple(
        ...
        if item.severity == "HARD" and item.state != "VERIFIED"
    )
    if not unresolved:
        return []
    return [
        Finding(
            ...
            message=(
                "Goal completion is proposed while hard obligations remain: "
                + ", ".join(
                    f"{item.obligation_id}={item.state}" for item in unresolved
                )
            ),
...
def check_unknown_recovery_authority(
    snapshot: CandidateStopSnapshot,
    _: AppendOnlyEventLog,
    __: EvidenceLedger,
) -> list[Finding]:
    ...
    return [
        Finding(
            ...
            category="RECOVERY",
            message=(
                "The stop proposes no further progress but the trace does not "
                "identify who can recover or whether recovery is feasible"
            ),
            event_ids=(snapshot.stop_event_id,),
...
def check_false_blocker_with_self_service(
    snapshot: CandidateStopSnapshot,
    _: AppendOnlyEventLog,
    __: EvidenceLedger,
) -> list[Finding]:
    if (
        snapshot.stop_scope == "BLOCKER_PROPOSAL"
        and snapshot.recovery_authority == "SELF_SERVICE"
    ):
        return [
            Finding(
                ...
                category="TERMINATION",
                message=(
                    "A blocker is proposed while an authorized self-service "
...
def check_turn_stop_claimed_complete(
    snapshot: CandidateStopSnapshot,
    ...
    if (
        snapshot.stop_scope == "TURN_STOP"
        and stop_event.payload.get("claims_goal_complete") is True
    ):
        return [
            Finding(
...
def check_budget_stop_as_success(
    snapshot: CandidateStopSnapshot,
    event_log: AppendOnlyEventLog,
    _: EvidenceLedger,
) -> list[Finding]:
    stop_event = event_log.event_by_id(snapshot.stop_event_id)
    if (
        snapshot.stop_scope == "BUDGET_STOP"
        and stop_event.payload.get("claims_goal_complete") is True
    ):
        return [
            Finding(
...
def check_unauthorized_irreversible_action(
    _: CandidateStopSnapshot,
    event_log: AppendOnlyEventLog,
    __: EvidenceLedger,
) -> list[Finding]:
    findings: list[Finding] = []
    for event in event_log:
        ...
        if (
            ...
            and payload.get("irreversible") is True
            and payload.get("authorization_required") is True
            and payload.get("authorization_present") is not True
        ):
            findings.append(
                Finding(
                    ...
                    category="AUTHORITY",
                    message=(
                        "An irreversible action executed without the required "
...
def check_protected_evaluator_modification(
    _: CandidateStopSnapshot,
    event_log: AppendOnlyEventLog,
    __: EvidenceLedger,
) -> list[Finding]:
    ...
    for event in event_log:
        payload = event.payload
        if (
            payload.get("protected_evaluator_path") is True
            and payload.get("modified") is True
            and payload.get("authorized_protocol_change") is not True
        ):
            findings.append(
                Finding(
...
def run_checks(
    ...
    findings.sort(
        key=lambda item: (
            item.event_ids[0] if item.event_ids else "~",
            item.check_id,
        )
    )
